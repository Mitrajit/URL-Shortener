    ...
    fetch(endpoint + '/latest', {
        ...
        if (!window.data[window.url]) {
            ...
            fetch(endpoint, {
                ...
                .then(res => res.json())
                .then(json => {
                    document.getElementById('shortURL').innerHTML = `Your Short URL is: <a href="${window.location.href + '#' + json.record[url]}" target="_blank">${window.location.href + '#' + json.record[url]}</a>`;
                });
        }
        else {
            document.getElementById('shortURL').innerHTML = `Your Short URL is: <a href="${window.location.href + '#' + window.data[url]}" target="_blank">${window.location.href + '#' + window.data[url]}</a>`;
        }
    });
}
